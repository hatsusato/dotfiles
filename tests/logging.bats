...
@test "LOG-01: log_debug function exists and is callable" {
	run bash -c 'source lib/logging.sh && type log_debug'
	assert_success
...
@test "LOG-02: log_info function exists and is callable" {
	run bash -c 'source lib/logging.sh && type log_info'
	assert_success
...
@test "LOG-03: log_warn function exists and is callable" {
	run bash -c 'source lib/logging.sh && type log_warn'
	assert_success
...
@test "LOG-04: log_error function exists and is callable" {
	run bash -c 'source lib/logging.sh && type log_error'
	assert_success
...
@test "LOG-05b: with LOG_LEVEL=debug, info messages output" {
	run bash -c 'source lib/logging.sh; export LOG_LEVEL=debug LOG_PREFIX=test; log_info "info msg" 2>&1'
	assert_success
	assert_output --partial 'info msg'
}

@test "LOG-05c: with LOG_LEVEL=debug, warn messages output" {
	run bash -c 'source lib/logging.sh; export LOG_LEVEL=debug LOG_PREFIX=test; log_warn "warn msg" 2>&1'
	assert_success
	assert_output --partial 'warn msg'
}

@test "LOG-05d: with LOG_LEVEL=debug, error messages output" {
	run bash -c 'source lib/logging.sh; export LOG_LEVEL=debug LOG_PREFIX=test; log_error "error msg" 2>&1'
	assert_success
...
@test "LOG-06: with LOG_LEVEL=info, debug messages suppressed" {
	run bash -c 'source lib/logging.sh; export LOG_LEVEL=info LOG_PREFIX=test; log_debug "debug msg" 2>&1'
	assert_success
...
@test "LOG-07: with LOG_LEVEL=warn, debug messages suppressed" {
	run bash -c 'source lib/logging.sh; export LOG_LEVEL=warn LOG_PREFIX=test; log_debug "debug msg" 2>&1'
	assert_success
	refute_output --partial 'debug msg'
}

@test "LOG-07b: with LOG_LEVEL=warn, info messages suppressed" {
	run bash -c 'source lib/logging.sh; export LOG_LEVEL=warn LOG_PREFIX=test; log_info "info msg" 2>&1'
	assert_success
...
@test "LOG-08: with LOG_LEVEL=error, only error messages output" {
	run bash -c 'source lib/logging.sh; export LOG_LEVEL=error LOG_PREFIX=test; log_debug "debug msg" 2>&1'
	assert_success
	refute_output --partial 'debug msg'
}

@test "LOG-08b: with LOG_LEVEL=error, info messages suppressed" {
	run bash -c 'source lib/logging.sh; export LOG_LEVEL=error LOG_PREFIX=test; log_info "info msg" 2>&1'
	assert_success
	refute_output --partial 'info msg'
}

@test "LOG-08c: with LOG_LEVEL=error, warn messages suppressed" {
	run bash -c 'source lib/logging.sh; export LOG_LEVEL=error LOG_PREFIX=test; log_warn "warn msg" 2>&1'
	assert_success
...
@test "LOG-10: LOG_PREFIX environment variable is included in output" {
	run bash -c 'source lib/logging.sh; export LOG_PREFIX=mymodule LOG_LEVEL=info; log_info "test message" 2>&1'
	assert_success
...
@test "LOG-11: LOG_PREFIX format includes brackets [prefix]" {
	run bash -c 'source lib/logging.sh; export LOG_PREFIX=mymodule LOG_LEVEL=info; log_info "message" 2>&1'
	assert_success
	assert_output --partial '[mymodule]'
}
...
@test "LOG-12: LOG_PREFIX persists across multiple calls" {
	run bash -c 'source lib/logging.sh; export LOG_PREFIX=persistent LOG_LEVEL=info; (log_info "first" 2>&1; log_warn "second" 2>&1) | grep persistent'
	assert_success
}
...
@test "LOG-13d: output format includes MESSAGE" {
	run bash -c 'source lib/logging.sh; export LOG_LEVEL=info LOG_PREFIX=deploy; log_info "test message" 2>&1'
	assert_success
	assert_output --partial 'test message'
}
...
@test "LOG-16: log_debug outputs to stderr, not stdout" {
	run bash -c 'source lib/logging.sh; export LOG_LEVEL=debug LOG_PREFIX=test; log_debug "msg" 2>/dev/null'
	assert_success
	assert_output ""
}

@test "LOG-16b: log_debug message visible on stderr" {
	run bash -c 'source lib/logging.sh; export LOG_LEVEL=debug LOG_PREFIX=test; log_debug "stderr msg" 2>&1 >/dev/null'
	assert_success
...
@test "LOG-16a: log_info outputs to stderr, not stdout" {
	run bash -c 'source lib/logging.sh; export LOG_LEVEL=info LOG_PREFIX=test; log_info "msg" 2>/dev/null'
	assert_success
	assert_output ""
}

@test "LOG-16a2: log_info message visible on stderr" {
	run bash -c 'source lib/logging.sh; export LOG_LEVEL=info LOG_PREFIX=test; log_info "stderr msg" 2>&1 >/dev/null'
	assert_success
	assert_output --partial 'stderr msg'
}
...
@test "LOG-16b: log_warn outputs to stderr, not stdout" {
	run bash -c 'source lib/logging.sh; export LOG_LEVEL=warn LOG_PREFIX=test; log_warn "msg" 2>/dev/null'
	assert_success
	assert_output ""
}

@test "LOG-16b2: log_warn message visible on stderr" {
	run bash -c 'source lib/logging.sh; export LOG_LEVEL=warn LOG_PREFIX=test; log_warn "stderr msg" 2>&1 >/dev/null'
	assert_success
	assert_output --partial 'stderr msg'
}
...
@test "LOG-16c: log_error outputs to stderr, not stdout" {
	run bash -c 'source lib/logging.sh; export LOG_LEVEL=error LOG_PREFIX=test; log_error "msg" 2>/dev/null'
	assert_success
	assert_output ""
}

@test "LOG-16c2: log_error message visible on stderr" {
	run bash -c 'source lib/logging.sh; export LOG_LEVEL=error LOG_PREFIX=test; log_error "stderr msg" 2>&1 >/dev/null'
	assert_success
	assert_output --partial 'stderr msg'
}
