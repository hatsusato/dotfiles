...
@test "LOG-09: default LOG_LEVEL equals info (debug suppressed)" {
	run bash -c 'unset LOG_LEVEL; source ~/.local/lib/logging.sh; export LOG_PREFIX=test; log_debug "debug msg" 2>&1'
	assert_success
	refute_output --partial 'debug msg'
}

@test "LOG-09b: default LOG_LEVEL equals info (info output)" {
	run bash -c 'unset LOG_LEVEL; source ~/.local/lib/logging.sh; export LOG_PREFIX=test; log_info "info msg" 2>&1'
	assert_success
	assert_output --partial 'info msg'
}

@test "LOG-09c: default LOG_LEVEL equals info (warn output)" {
	run bash -c 'unset LOG_LEVEL; source ~/.local/lib/logging.sh; export LOG_PREFIX=test; log_warn "warn msg" 2>&1'
	assert_success
	assert_output --partial 'warn msg'
}

@test "LOG-09d: default LOG_LEVEL equals info (error output)" {
	run bash -c 'unset LOG_LEVEL; source ~/.local/lib/logging.sh; export LOG_PREFIX=test; log_error "error msg" 2>&1'
	assert_success
	assert_output --partial 'error msg'
}
